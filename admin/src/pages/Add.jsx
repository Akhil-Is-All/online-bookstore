...
const Add = ({token}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div>
        <p className='mb-2'>Product edition</p>
        <div className='flex gap-3'>
          ...
          <div onClick={()=>setSizes(prev=>prev.includes("2")?prev.filter(item=>item!=="2"):[...prev,"2"])}>
            <p className={`${sizes.includes("2")?"bg-pink-100":"bg-slate-200"} px-3 py-1 cursor-pointer  `}>2</p>
          </div>
          <div onClick={()=>setSizes(prev=>prev.includes("3")?prev.filter(item=>item!=="3"):[...prev,"3"])}>
            <p className={`${sizes.includes("3")?"bg-pink-100":"bg-slate-200"} px-3 py-1 cursor-pointer  `}>3</p>
          </div>
        </div>
      </div>
